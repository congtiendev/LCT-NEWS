        ...
        <article class="w-full h-full px-4 py-3 bg-white shadow-xs sm:px-5 sm:py-3">
            ...
            <div class="flex flex-col gap-3 my-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <nav>
                    <ol class="flex items-center gap-2 text-sm">
                        ...
                        <li class="text-primary">/Chỉnh sửa bài viết</li>
                    </ol>
                </nav>
            </div>
            <!-- Breadcrumb End -->
            ...
            <section class="w-full h-full setting-user-info">
                <form method="post" action="{{ route('admin.post.update',['id'=>$post->id]) }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                        <div class="w-full">
                            <div class="mb-5.5">
                                ...
                                <input class="w-full p-3 font-medium text-black border rounded border-stroke bg-gray focus:border-primary focus-visible:outline-none" type="text" name="title" placeholder="Nhập tiêu đề..." value="{{ old('title') ? old('title') : $post->title }}" />
                                @error('title')
                            ...
                            <div class="mb-5.5">
                                ...
                                <textarea class="w-full p-3 font-medium text-black border rounded resize-none border-stroke bg-gray focus:border-primary focus-visible:outline-none" name="description" rows="6" placeholder="Nhập mô tả...">{{ old('title') ? old('title') : $post->description}}</textarea>
                                @error('description')
                                <span class="text-sm text-red-500">{{ $message }}</span>
                                @enderror
                            </div>
                    ...
                    <label>
                        <textarea id="content__post" name="content" class="" placeholder="Nội dung bài viết...">{{ old('content') ? old('content') : $post->content }}</textarea>
                        @error('content')
                        <span class="text-sm text-red-500">{{ $message }}</span>
                        @enderror
                    </label>
